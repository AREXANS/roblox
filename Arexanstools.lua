--[[
    Skrip diperbaiki untuk stabilitas dan kompatibilitas yang lebih baik.
    Perubahan:
    1. Dibungkus dalam task.spawn() untuk mencegah pemblokiran thread executor.
    2. Penanganan error yang lebih baik untuk fungsi spesifik executor (writefile, readfile, setclipboard) dengan notifikasi dalam game.
    3. Panggilan 'require' untuk modul mobile dibungkus dalam pcall untuk mencegah error jika gagal.
    4. Menghapus referensi 'script:Destroy()' yang tidak perlu pada fungsi CloseScript.
]]
...
task.spawn(function()
    ...
    updateTeleportList = function()
        ...
        TeleportTabContent.CanvasSize = UDim2.new(0, 0, 0, TeleportListLayout.AbsoluteContentSize.Y) 
    end
    ...
    local function StartFly()
        ...
        local bodyGyro = Instance.new("BodyGyro", root); bodyGyro.Name = "FlyGyro"; bodyGyro.P = 9e4; bodyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bodyGyro.CFrame = root.CFrame
        local bodyVelocity = Instance.new("BodyVelocity", root); bodyVelocity.Name = "FlyVelocity"; bodyVelocity.MaxForce = Vector3.new(9e9, 9e9, 9e9); bodyVelocity.Velocity = Vector3.new(0, 0, 0)
    ...
    local function StartMobileFly()
        ...
        humanoid.PlatformStand = true
        local bodyVelocity = Instance.new("BodyVelocity", root); bodyVelocity.Name = "FlyVelocity"; bodyVelocity.MaxForce = Vector3.new(9e9, 9e9, 9e9); bodyVelocity.Velocity = Vector3.new(0, 0, 0)
        local bodyGyro = Instance.new("BodyGyro", root); bodyGyro.Name = "FlyGyro"; bodyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bodyGyro.P = 1000; bodyGyro.D = 50
    ...
    local function CreateTouchFlingGUI()
        ...
        local FlingScreenGui = Instance.new("ScreenGui"); FlingScreenGui.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"); FlingScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; FlingScreenGui.ResetOnSpawn = false
        touchFlingGui = FlingScreenGui
    
        local Frame = Instance.new("Frame", FlingScreenGui); Frame.BackgroundColor3 = Color3.fromRGB(170, 200, 255); Frame.BackgroundTransparency = 0.3; Frame.BorderSizePixel = 0; Frame.Position = UDim2.new(0.5, -45, 0.5, -28); Frame.Size = UDim2.new(0, 90, 0, 56); Frame.Active = true; Frame.Draggable = true
        local FrameUICorner = Instance.new("UICorner", Frame); FrameUICorner.CornerRadius = UDim.new(0, 6)
        local FrameUIStroke = Instance.new("UIStroke", Frame); FrameUIStroke.Color = Color3.fromRGB(0, 100, 255); FrameUIStroke.Thickness = 1.5; FrameUIStroke.Transparency = 0.2
        local TitleBar = Instance.new("Frame", Frame); TitleBar.BackgroundColor3 = Color3.fromRGB(140, 170, 235); TitleBar.BackgroundTransparency = 0.4; TitleBar.BorderSizePixel = 0; TitleBar.Size = UDim2.new(1, 0, 0, 18)
        ...
        local OnOffButton = Instance.new("TextButton", Frame); OnOffButton.BackgroundColor3 = Color3.fromRGB(0, 120, 255); OnOffButton.BorderSizePixel = 0; OnOffButton.Position = UDim2.new(0.5, -30, 0, 25); OnOffButton.Size = UDim2.new(0, 60, 0, 22); OnOffButton.Font = Enum.Font.SourceSansBold; OnOffButton.Text = "OFF"; OnOffButton.TextColor3 = Color3.fromRGB(255, 255, 255); OnOffButton.TextSize = 14
        local OnOffButtonCorner = Instance.new("UICorner", OnOffButton); OnOffButtonCorner.CornerRadius = UDim.new(0, 5)
        local OnOffButtonGradient = Instance.new("UIGradient", OnOffButton); OnOffButtonGradient.Color = ColorSequence.new(Color3.fromRGB(100, 180, 255), Color3.fromRGB(80, 150, 255)); OnOffButtonGradient.Rotation = 90
        ...
        OnOffButton.MouseButton1Click:Connect(function()
            hiddenfling = not hiddenfling; OnOffButton.Text = hiddenfling and "ON" or "OFF"
            if hiddenfling then if not flingThread or coroutine.status(flingThread) == "dead" then flingThread = coroutine.create(fling); coroutine.resume(flingThread) end end
        end)
    
        CloseButton.MouseButton1Click:Connect(function()
            hiddenfling = false; FlingScreenGui:Destroy(); touchFlingGui = nil
        end)
    end
    ...
    local function ToggleAimbot(enabled)
        ...
        if enabled then
            ...
            AimbotConnection = RunService.RenderStepped:Connect(function()
                local camera = Workspace.CurrentCamera; local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                if not (root and camera) then return end
                local mousePos = UserInputService:GetMouseLocation(); local closestNPC, closestDistance = nil, Settings.AimbotFOV
                for _, npc in pairs(Workspace:GetDescendants()) do
                    if npc:IsA("Model") and npc ~= LocalPlayer.Character and npc:FindFirstChildOfClass("Humanoid") and npc:FindFirstChild(Settings.AimbotPart) then
                        ...
                        if humanoid.Health > 0 then
                            ...
                            if onScreen then
                                ...
                                if distance <= closestDistance then closestDistance, closestNPC = distance, npc end
                            end
                        end
                    end
                end
                AimbotTarget = closestNPC
                if AimbotTarget and AimbotTarget:FindFirstChild(Settings.AimbotPart) then camera.CFrame = CFrame.new(camera.CFrame.Position, AimbotTarget[Settings.AimbotPart].Position); AimbotTarget.Humanoid:TakeDamage(Settings.KillAuraDamage) end
    ...
    local function ToggleAntiLag(enabled)
        ...
        if enabled then
            if not OriginalGraphicsSettings.Technology then
                OriginalGraphicsSettings.Technology = Lighting.Technology; OriginalGraphicsSettings.GlobalShadows = Lighting.GlobalShadows; OriginalGraphicsSettings.QualityLevel = currentSettings.Rendering.QualityLevel; OriginalGraphicsSettings.MaterialQuality = MaterialService.MaterialQuality; OriginalGraphicsSettings.Descendants = {}
                for _, v in pairs(Lighting:GetDescendants()) do if v:IsA("PostEffect") then OriginalGraphicsSettings.Descendants[v] = v.Enabled end end
                if Workspace:FindFirstChild("Terrain") then OriginalGraphicsSettings.Decoration = Workspace.Terrain.Decoration; OriginalGraphicsSettings.WaterWaveSize = Workspace.Terrain.WaterWaveSize; OriginalGraphicsSettings.WaterWaveSpeed = Workspace.Terrain.WaterWaveSpeed; OriginalGraphicsSettings.WaterReflectance = Workspace.Terrain.WaterReflectance end
            end
    
            Lighting.Technology = Enum.Technology.Compatibility; Lighting.GlobalShadows = false; currentSettings.Rendering.QualityLevel = Enum.QualityLevel.Level01; MaterialService.MaterialQuality = Enum.MaterialQuality.Low
            for _, v in pairs(Lighting:GetDescendants()) do if v:IsA("PostEffect") then v.Enabled = false end end
            if Workspace:FindFirstChild("Terrain") then Workspace.Terrain.Decoration = false; Workspace.Terrain.WaterWaveSize = 0; Workspace.Terrain.WaterWaveSpeed = 0; Workspace.Terrain.WaterReflectance = 0 end
    
        else
            if OriginalGraphicsSettings.Technology then
                Lighting.Technology = OriginalGraphicsSettings.Technology; Lighting.GlobalShadows = OriginalGraphicsSettings.GlobalShadows; currentSettings.Rendering.QualityLevel = OriginalGraphicsSettings.QualityLevel; MaterialService.MaterialQuality = OriginalGraphicsSettings.MaterialQuality
                for v, originalState in pairs(OriginalGraphicsSettings.Descendants) do if v and v.Parent then v.Enabled = originalState end end
                if Workspace:FindFirstChild("Terrain") then if OriginalGraphicsSettings.Decoration ~= nil then Workspace.Terrain.Decoration = OriginalGraphicsSettings.Decoration end; if OriginalGraphicsSettings.WaterWaveSize ~= nil then Workspace.Terrain.WaterWaveSize = OriginalGraphicsSettings.WaterWaveSize end; if OriginalGraphicsSettings.WaterWaveSpeed ~= nil then Workspace.Terrain.WaterWaveSpeed = OriginalGraphicsSettings.WaterWaveSpeed end; if OriginalGraphicsSettings.WaterReflectance ~= nil then Workspace.Terrain.WaterReflectance = OriginalGraphicsSettings.WaterReflectance end end
                OriginalGraphicsSettings = {}
            end
        end
    end
    ...
    LocalPlayer.CharacterAdded:Connect(function(character)
        task.wait(0.1)
        if character:FindFirstChildOfClass("Humanoid") then character.Humanoid.WalkSpeed = IsWalkSpeedEnabled and Settings.WalkSpeed or OriginalWalkSpeed end
    ...
    local function updatePlayerList()
        if not PlayerTabContent.Visible then return end
        ...
        for _, child in pairs(PlayerListContainer:GetChildren()) do if child:IsA("Frame") and child.Name ~= "UIListLayout" then child:Destroy() end end
        for i, player in ipairs(playersToShow) do
            ...
            local avatarImage = Instance.new("ImageLabel", playerFrame); avatarImage.Size = UDim2.new(0, 25, 0, 25); avatarImage.Position = UDim2.new(0, 5, 0.5, -12.5); avatarImage.BackgroundTransparency = 1
            local success, _ = pcall(function() avatarImage.Image = Players:GetUserThumbnailAsync(player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420) end)
            if not success then warn("Gagal memuat thumbnail untuk", player.Name) end
            local displaynameLabel = Instance.new("TextLabel", playerFrame); displaynameLabel.Size = UDim2.new(0.6, -20, 0, 15); displaynameLabel.Position = UDim2.new(0, 35, 0, 2); displaynameLabel.BackgroundTransparency = 1; displaynameLabel.TextXAlignment = Enum.TextXAlignment.Left; displaynameLabel.Text = player.DisplayName; displaynameLabel.TextColor3 = Color3.fromRGB(255, 255, 255); displaynameLabel.TextSize = 11; displaynameLabel.Font = Enum.Font.SourceSansSemibold
            local usernameLabel = Instance.new("TextLabel", playerFrame); usernameLabel.Size = UDim2.new(0.6, -20, 0, 15); usernameLabel.Position = UDim2.new(0, 35, 0, 16); usernameLabel.BackgroundTransparency = 1; usernameLabel.TextXAlignment = Enum.TextXAlignment.Left; usernameLabel.Text = "@" .. player.Name; usernameLabel.TextColor3 = Color3.fromRGB(150, 150, 150); usernameLabel.TextSize = 9; usernameLabel.Font = Enum.Font.SourceSans
            local distanceLabel = Instance.new("TextLabel", playerFrame); distanceLabel.Name = "DistanceLabel"; distanceLabel.Size = UDim2.new(0, 40, 0, 20); distanceLabel.Position = UDim2.new(1, -90, 0.5, -10); distanceLabel.BackgroundTransparency = 1; distanceLabel.TextXAlignment = Enum.TextXAlignment.Right; distanceLabel.TextColor3 = Color3.fromRGB(0, 255, 127); distanceLabel.TextSize = 10; distanceLabel.Font = Enum.Font.SourceSansSemibold
            ...
            teleportButton.Size = UDim2.new(0, 40, 0, 20); teleportButton.Position = UDim2.new(1, -45, 0.5, -10); teleportButton.TextSize = 10
            ...
            distanceLabel.Text = (localHRP and targetHRP) and tostring(math.floor((localHRP.Position - targetHRP.Position).Magnitude)) .. "m" or "..."
        end
        ...
        showImportPrompt(function(text)
            ...
            for _, data in ipairs(decodedData) do
                if type(data) == "table" and data.Name and data.CFrameData and not existingNames[data.Name] then
                    ...
                    table.insert(savedTeleportLocations, { Name = data.Name, CFrame = cframe }); existingNames[data.Name] = true; importedCount = importedCount + 1
                end
            end
    ...
    local function MakeDraggable(guiObject, dragHandle)
        dragHandle.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local isDragging = false; local dragStartMousePos = input.Position; local startObjectPos = guiObject.Position
                local inputChangedConnection; local inputEndedConnection
                ...
                inputChangedConnection = UserInputService.InputChanged:Connect(function(changedInput)
                    if changedInput.UserInputType == input.UserInputType then
                        ...
                        if not isDragging and delta.Magnitude > DRAG_THRESHOLD then isDragging = true end
                        if isDragging then guiObject.Position = UDim2.new(startObjectPos.X.Scale, startObjectPos.X.Offset + delta.X, startObjectPos.Y.Scale, startObjectPos.Y.Offset + delta.Y) end
                    end
                end)
                     ...
                     if endedInput.UserInputType == input.UserInputType then
                        ...
                        if dragHandle == MiniToggleButton and not isDragging then
                            ...
                            if MainFrame.Visible then
                                if not (PlayerTabContent.Visible or GeneralTabContent.Visible or CombatTabContent.Visible or TeleportTabContent.Visible or SettingsTabContent.Visible) then switchTab("Player") end
                                if PlayerTabContent.Visible then updatePlayerList() end
                            end
                        end
                     end
                end)
